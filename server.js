...
function parsePromotionPayload(body) {
  ...
  if (!categoria) {
    return {
      erro:
        'Selecione a categoria da promoção.'
    };
  }
  ...
  if (
    ...
    return {
      erro:
        'Paredão deve ter pelo menos 60 minutos.'
    };
  }
  ...
  if (
    ...
    return {
      erro:
        'O horário fixo deve estar no formato HH:MM.'
    };
  }
  ...
  if (
    ...
    return {
      erro:
        'A imagem precisa ser PNG, JPG ou WebP em base64.'
    };
  }
  ...
  if (
    ...
    return {
      erro:
        'A imagem da promoção está muito grande.'
    };
  }
...
function validateServiceOrPromotion(
  body,
  promo
) {
...
async function validateCheckout(
  ...
  if (
    ...
    throw new Error(
      'A data escolhida não é válida para esta promoção.'
    );
  }
  ...
  if (conflict) {
    throw new Error(
      'Esse horário entra em conflito com outro agendamento. Escolha outro horário.'
    );
  }
...
async function deactivatePromotionById(
  id
) {
  if (!id) {
    return null;
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .json({
            erro:
              'ID de promoção inválido.'
          });
      }

      const updated =
        await deactivatePromotionById(
          id
        );
      ...
      return res
        ...
        .json({
          erro:
            'Não foi possível apagar a promoção.'
        });
    }
  }
);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      payment.agendamento.checkoutToken =
        saved.checkoutToken;
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .json({
            erro:
              'Este agendamento já foi atualizado.'
          });
      }
      ...
      if (
        ...
        return res
          ...
          .json({
            erro:
              'A reserva temporária expirou. Faça um novo agendamento.'
          });
      }
  ...
  async (req, res) => {
    try {
      ...
      return res
        ...
        .json({
          erro:
            'Não foi possível registrar o pagamento informado.'
        });
    }
  }
);
    ...
    try {
      ...
      return res
        ...
        .json({
          erro:
            error.message ||
            'Não foi possível criar o agendamento manual.'
        });
    }
  }
);
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .json({
            erro:
              'Só é possível confirmar depois que o cliente informar o pagamento e o atendimento conferir o Pix.'
          });
      }
...
async function connectDatabase() {
  if (!MONGO_URI) {
    if (
      ...
      throw new Error(
        'MONGO_URI não configurado. O ambiente de produção não pode usar memória temporária.'
      );
    }

    console.warn(
      '[DB] MONGO_URI ausente. Rodando em memória apenas no desenvolvimento.'
    );
...
async function start() {
  if (
    ...
    throw new Error(
      'ADMIN_PASSWORD e ADMIN_SESSION_SECRET são obrigatórios em produção.'
    );
  }
  ...
  app.listen(
    ...
    () =>
      console.log(
        `Pauluzzi Bronze rodando na porta ${PORT}`
      )
  );
}
